...
<html lang="en">
  ...
  <body>
    ...
    <p class="center">
      <a href="https://parisairwaysguna.onrender.com/ParisAirways/v1/verfyToken/${token}/${email}" class="ancor"> Reset Password </a>
    </p>
    <p class="para">
  ...
  try {
    ...
    if (userEmail.isexpire <= time) {
      ...
      if (token === userEmail.resetToken) {
        ...
        return res.redirect(
          200,
          `https://parisairwaysguna.onrender.com/ParisAirways/v1/resetPassword/${email}`
        );
      } else {
        res.status(401).json({
          status: "error",
  ...
  try {
    ...
    if (!user) {
      ...
      if (verify === false) {
        ...
        res.cookie(
          "profile",
          `https://parisairwaysguna.onrender.com/${user.profilePicture}`
        );
        res.status(200).json({
          status: "Success",
...
exports.checkOut = async (req, res, next) => {
  ...
  const myDomain = "https://parisairwaysguna.onrender.com";
  const payment = {
